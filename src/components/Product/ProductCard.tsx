...
const ProductCard: React.FC<ProductCardProps> = ({ product, onQuickView }) => {
  ...
  return (
    <Link 
      ...
      className="group relative bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 block"
    >
      {/* Image Container */}
      <div className="relative aspect-[4/5] overflow-hidden bg-gray-100">
        ...
        <div className="absolute top-4 right-4 flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          ...
          <button
            ...
            className="w-10 h-10 bg-white text-gray-600 rounded-full flex items-center justify-center hover:bg-gray-50 transition-colors duration-200"
          >
        ...
        <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button
            ...
            className="w-full bg-white text-gray-900 py-2 px-4 rounded-lg font-medium hover:bg-gray-50 transition-colors duration-200 flex items-center justify-center space-x-2"
          >
        ...
        <div className="mb-3">
          <div className="flex items-center space-x-1">
            ...
            {product.size.length > 4 && (
              <span className="text-xs text-gray-400">+{product.size.length - 4}</span>
            )}
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="flex items-center space-x-2">
            ...
            {product.color.length > 3 && (
              <span className="text-xs text-gray-400">+{product.color.length - 3}</span>
            )}
          </div>
        </div>
